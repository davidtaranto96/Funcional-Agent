...
function dateToARDay(date: Date): string {
  return new Intl.DateTimeFormat('en-CA', { timeZone: TZ }).format(date);
}

function toArgentinaISO(date: Date): string {
    ...
    if (!isWeekend) {
      ...
      if (dayEnd > now) {
        ...
        for (const hour of CANDIDATE_HOURS) {
          ...
          const isBusy = busy.some(b => b.start && b.end && slotStart < new Date(b.end) && slotEnd > new Date(b.start));
...
export function formatSlotsForWhatsApp(slots: Slot[]): string {
...
export async function createMeetingEvent(slot: Slot, clientName: string, clientEmail: string | null) {
  if (!process.env.GOOGLE_REFRESH_TOKEN) throw new Error('GOOGLE_REFRESH_TOKEN no configurado');
  const auth = getOAuth2Client();
  const calendar = google.calendar({ version: 'v3', auth });
  ...
  const event: calendar_v3.Schema$Event = {
    ...
    conferenceData: {
      createRequest: { requestId: `dtsy-${Date.now()}`, conferenceSolutionKey: { type: 'hangoutsMeet' } },
    },
...
export async function getUpcomingMeetings(maxResults = 10): Promise<UpcomingMeeting[]> {
...
export function formatMeetingDate(isoString: string) {
